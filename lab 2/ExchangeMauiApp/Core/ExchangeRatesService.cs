...
namespace ExchangeMauiApp.Core
{
    public class ExchangeRatesService
    {
        ...
        public async Task GetData(String date)
        {
            ...
            // Upewnij się, że baza istnieje (tworzy tabele)
        ...
        public void AddCountry(string countryName, string currencyCode, string date)
        {
            if (!DateTime.TryParse(date, out DateTime parsedDate)) return;
            AddCountryToCurrency(countryName, currencyCode, parsedDate);
        }
        ...
        public async Task<string> GetRateForCountry(string countryName, string date)
        {
            using var context = new ExchangeDbContext();
            if (!DateTime.TryParse(date, out var targetDate))
                return $"Nieprawidłowa data: {date}";
            ...
            if (country == null)
                return $"Kraj '{countryName}' nie istnieje w bazie.";
            ...
            if (rate == null)
            {
                ...
                // Ponowna próba po aktualizacji bazy
                using var retryContext = new ExchangeDbContext();
                var newRate = retryContext.Currencies
                    .FirstOrDefault(c => c.Code == country.Currency.Code && c.Date.Date == targetDate.Date);

                return newRate != null
                    ? $"{country.Name} ({newRate.Code}): {newRate.Rate} ({newRate.Date:yyyy-MM-dd})"
                    : $"Brak kursu waluty {country.Currency.Code} z dnia {date}";
            }

            return $"{country.Name} ({rate.Code}): {rate.Rate} ({rate.Date:yyyy-MM-dd})";
        }

        public List<string> GetAllCountriesWithCurrencies()
        {
            using var context = new ExchangeDbContext();
            var list = context.Countries
                .Include(c => c.Currency)
                .OrderBy(c => c.Name)
                .Select(c => $"{c.Name} - {c.Currency.Code}")
                .ToList();
